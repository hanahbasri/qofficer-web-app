    ...
    <div class="row g-3 mb-4">
        @forelse($roles as $role)
            ...
            <div class="col-sm-6 col-lg-3">
                <div class="card p-3 h-100" style="border-top:3px solid {{ $ri['color'] }}">
                    <div class="d-flex align-items-center gap-3 mb-3">
                        <div
                            style="width:40px;height:40px;border-radius:10px;background:{{ $ri['bg'] }};display:flex;align-items:center;justify-content:center;flex-shrink:0">
                            <i class="bi {{ $ri['icon'] }}" style="font-size:1.1rem;color:{{ $ri['color'] }}"></i>
                        </div>
                        <div>
                            <div class="fw-bold" style="font-size:.9rem;color:var(--text)">{{ $role->display_name }}</div>
                            <div class="font-monospace" style="font-size:.7rem;color:var(--text-muted)">{{ $role->name }}
                            </div>
                        </div>
                    </div>
                    <div class="mt-auto pt-2 border-top d-flex align-items-center justify-content-between">
                        <span style="font-size:.75rem;color:var(--text-muted)">Total pengguna</span>
                        <span class="fw-bold"
                            style="font-size:1.1rem;color:{{ $ri['color'] }}">{{ $role->users_count }}</span>
                    </div>
                </div>
            </div>
        @empty
